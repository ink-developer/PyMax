...
#### `send_message(text: str, chat_id: int, notify: bool, attachment: Photo | File | None = None, attachments: list[Photo | File] | None = None, reply_to: int | None = None, use_queue: bool = False) -> Message | None`
...
| `attachment` | `Photo \| File \| None` | Один файл/фото для прикрепления (опционально) |
| `attachments` | `list[Photo \| File] \| None` | Список файлов/фото для прикрепления (опционально) |
...
#### `edit_message(chat_id: int, message_id: int, text: str, attachment: Photo | None = None, attachments: list[Photo] | None = None, use_queue: bool = False) -> Message | None`
...
| `attachment` | `Photo \| None` | Одно фото для прикрепления (опционально) |
| `attachments` | `list[Photo] \| None` | Список фото для прикрепления (опционально) |
| `use_queue` | `bool` | Добавить в очередь вместо немедленной отправки. По умолчанию `False` |
